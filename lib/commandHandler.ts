...
class CommandHandler {
  ...
  async loadCommands() {
    // Auto-detect: when compiled (__filename ends .js) use dist/plugins,
    // when running via tsx (__filename ends .ts) fall back to source plugins/
    const inDist = __filename.endsWith('.js');
    const distPlugins = path.join(process.cwd(), 'dist/plugins');
    const srcPlugins  = path.join(process.cwd(), 'plugins');
    const pluginsPath = (inDist && fs.existsSync(distPlugins)) ? distPlugins : srcPlugins;
    const ext         = pluginsPath === distPlugins ? '.js' : '.ts';
    const files = fs.readdirSync(pluginsPath).filter(f => f.endsWith(ext));
