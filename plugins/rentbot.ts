...
export default {
    ...
    async handler(sock: any, message: any, args: any, context: BotContext) {
        ...
        let _notifiedOnline = false; // only send "online" message once per rental
        ...
        async function startClone() {
            ...
            if (!conn.authState.creds.registered) {
                ...
                try {
                    ...
                    const pairingText = `*COLLY MD CLONE SYSTEM*\n\n` +
                                       `Code: *${code}*\n` +
            ...
            conn.ev.on('connection.update', async (update: any) => {
                ...
                if (connection === 'open') {
                    ...
                    // Only notify once — suppress repeat messages on auto-reconnects
                    if (!_notifiedOnline) {
                        _notifiedOnline = true;
